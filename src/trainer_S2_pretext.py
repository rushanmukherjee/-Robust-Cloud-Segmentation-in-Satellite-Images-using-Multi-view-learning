...
import models
import dataset
from models import S2_CloudSeg_allpretext
...
LC_train = join(train_dataset_dir, "landcover_onehot.nc")
LC_val = join(val_dataset_dir, "landcover_onehot.nc")
LC_test = join(test_dataset_dir, "landcover_onehot.nc")
...
##Setup Weights for the losses
cloudseg_weight = configs['CLOUDSEG_WEIGHT']
LC_weight = configs['LC_WEIGHT']
ele_weight = configs['ELE_WEIGHT']
S1_weight = configs['S1_WEIGHT']
...
model = model_module(
    ...
    cloudseg_weight = cloudseg_weight,
    LC_weight = LC_weight,
