...
class UnetS2_adaptiveDropout(nn.Module):
    def __init__(
            self,  S2_input_channels: int = 13,
        ) -> None:

        super(UnetS2_adaptiveDropout, self).__init__()
    ...
    def forward(self, s2: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        enc2 = self.S2enc2(encP1)
        encP2 = self.pool(enc2)
        enc3 = self.S2enc3(encP2)
        encP3 = self.pool(enc3)
        enc4 = self.S2enc4(encP3)
        encP4 = self.pool(enc4)
        enc5 = self.S2enc5(encP4)
...
def main():
    ...
    arch_graph = draw_graph(
        ...
        graph_dir='LR',
        expand_nested=True,
        graph_name='UnetS2_adaptiveDropout',
        save_graph=True,
