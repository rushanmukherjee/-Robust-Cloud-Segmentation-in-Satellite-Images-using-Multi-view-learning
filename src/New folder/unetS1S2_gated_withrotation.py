...
class CloudSegmentation_gated_rotation(nn.Module):
    def __init__(
            self,  S2_input_channels: int = 13, S1_input_channels=3,
        ) -> None:

        super(CloudSegmentation_gated_rotation, self).__init__()
        
        ##Pooling Operations
        self.pool = nn.MaxPool2d(kernel_size=2,stride=2)
        
        ##S2 Encoder Operations
        self.S2enc1 = conv_block(ch_in=S2_input_channels,ch_out=64)
        self.S2enc2 = conv_block(ch_in=64,ch_out=128)
        self.S2enc3 = conv_block(ch_in=128,ch_out=256)
        self.S2enc4 = conv_block(ch_in=256,ch_out=512)
        self.S2enc5 = conv_block(ch_in=512,ch_out=1024)
        
        ##S1 Encoder Operations
        self.S1enc1 = conv_block(ch_in=S1_input_channels,ch_out=64)
        self.S1enc2 = conv_block(ch_in=64,ch_out=128)
        self.S1enc3 = conv_block(ch_in=128,ch_out=256)
        self.S1enc4 = conv_block(ch_in=256,ch_out=512)
        self.S1enc5 = conv_block(ch_in=512,ch_out=1024)

        ## Call the fusion module
        #self.bottleneck_list = [self.S1enc5, self.S2enc5]
        self.fusion_mod_bottleneck = GatedFusionModule([1024,1024])

        ##Decoder operations
        self.dec_upsample5 = up_conv(ch_in=1024,ch_out=1024)
        self.fusion_mod5 = GatedFusionModule([1024,1024,1024])
        self.dec_conv5 = conv_block(ch_in=1024,ch_out=512)

        self.dec_upsample4 = up_conv(ch_in=512,ch_out=512)
        self.fusion_mod4 = GatedFusionModule([512,512,512])
        self.dec_conv4 = conv_block(ch_in=512,ch_out=256)

        self.dec_upsample3 = up_conv(ch_in=256,ch_out=256)
        self.fusion_mod3 = GatedFusionModule([256,256,256])
        self.dec_conv3 = conv_block(ch_in=256,ch_out=128)

        self.dec_upsample2 = up_conv(ch_in=128,ch_out=128)
        self.fusion_mod2 = GatedFusionModule([128,128,128])
        self.dec_conv2 = conv_block(ch_in=128,ch_out=64)

        self.dec_upsample1 = up_conv(ch_in=64,ch_out=64)
        self.fusion_mod1 = GatedFusionModule([64,64,64])
        self.dec_conv1 = conv_block(ch_in=64,ch_out=16)

        self.output = nn.Conv2d(in_channels=16, out_channels=4, kernel_size=3, stride=1, padding=1)
...
def main():
    ...
    arch = CloudSegmentation_gated_rotation()
    input_size = (
