...
s2_train = join(train_dataset_dir, "s2L1C_withrotated.nc")
s2_val = join(val_dataset_dir, "s2L1C.nc")
s2_test = join(test_dataset_dir, "s2L1C.nc")

s1_train = join(train_dataset_dir, "s1_withrotated.nc")
s1_val = join(val_dataset_dir, "s1.nc")
s1_test = join(test_dataset_dir, "s1.nc")

labels_train = join(train_dataset_dir, "HQlabels_withrotated.nc")
...
train_dataset = dataset_mod(label_dir=labels_train, s1_dir=s1_train, s2_dir=s2_train)
print(100*'-')
print(f"Number of data samples in training set = {len(train_dataset)}")
print(100*'-')
val_dataset = dataset_mod(label_dir=labels_val, s1_dir=s1_train, s2_dir=s2_val)
print(f"Number of data samples in validation set = {len(val_dataset)}")
print(100*'-')
test_dataset = dataset_mod(label_dir=labels_test, s1_dir=s1_train, s2_dir=s2_test)
print(f"Number of data samples in test set = {len(test_dataset)}")


# for idx in range(len(train_dataset)):
#     input,target = train_dataset[idx]

#     print(f"shape of input is {input['s2'].shape}")
#     print(f"shape of target is {target['cloud_labels'].shape}")
#     print(target)
