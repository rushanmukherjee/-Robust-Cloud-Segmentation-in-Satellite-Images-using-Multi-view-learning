...
train_dataset = dataset_mod(rotate_flag = True, label_dir=labels_train,s1_dir=s1_train) # s2_dir=s2_train) 
print(100*'-')
print(f"Number of data samples in training set = {len(train_dataset)}")
print(100*'-')
val_dataset = dataset_mod(rotate_flag = False, label_dir=labels_val,s1_dir=s1_train) # s2_dir=s2_val) 
print(f"Number of data samples in validation set = {len(val_dataset)}")
print(100*'-')
test_dataset = dataset_mod(rotate_flag = False, label_dir=labels_test, s1_dir=s1_train) #s2_dir=s2_test) 
...
with mlflow.start_run(run_id=run_id):
    ...
    for batch_idx in range(len(predictions_test)):
        ...
        for i in range(predictions_test[batch_idx].shape[0]):
            ...
            test_image = test_dataset[current_index]['s1']
            
            print(f"Test Image shape: {test_image.shape}")

            ratio = np.divide(test_image[2], test_image[1], out = np.zeros_like(test_image[2]), where = test_image[1] != 0)
            ratio = np.clip(ratio, 0, 1)

            rgb = np.stack((test_image[2], test_image[1], ratio), axis=2)
            ...
            if current_index % 100 == 0:
                overlay_img.save(join(f"{local_save_dir}", "artifacts", f"test_batch_{batch_idx}_sample_{i}_overlay.png"))
   ...
   # assert(False)
    ...
    metrics_df.to_csv(join(f"{local_save_dir}", "artifacts", "Sen1adaptiveDropout_testMetrics.csv"))
    ...
    # for batch_idx in range(len(predictions_test)):
    #     for i in range(predictions_test[batch_idx]['cloud_labels'].detach().cpu().numpy().shape[0]):
    #         fig, ax = plt.subplots(1, 3, figsize=(15, 5))
    #         ax[0].imshow(predictions_test[batch_idx]['cloud_labels'][i].cpu().numpy())
    #         ax[0].set_title("Predicted Cloud Labels")
    #         ax[1].imshow(predictions_test[batch_idx]['s1'][i].cpu().numpy())
    #         ax[1].set_title("Sentinel-1")
    #         ax[2].imshow(predictions_test[batch_idx]['s2'][i].cpu().numpy())
    #         ax[2].set_title("Sentinel-2")
            
    #         fig_dir = join(f"{run_dir}", "artifacts", "test_predictions")
    #         Path(fig_dir).mkdir(parents=True, exist_ok=True)
    #         fig.savefig(join(fig_dir, f"test_batch_{batch_idx}_sample_{i}.png"))
    #         plt.close()
