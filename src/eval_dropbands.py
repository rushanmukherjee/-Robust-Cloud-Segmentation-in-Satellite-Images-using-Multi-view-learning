...
# Get Drop Band Count and Flag from config
DROP_BAND_COUNT = configs['DROP_BAND_COUNT']

# Set up train/val/test dataset objects
train_dataset = dataset_mod(drop_band_flag = False, drop_band_count = 0, rotate_flag = True, label_dir=labels_train, s1_dir=s1_train, s2_dir=s2_train, landcover_dir=LC_train, azimuth_dir=azimuth_train, wateroccurence_dir=wateroccurence_train, DEM_dir=ele_train)
print(100*'-')
print(f"Number of data samples in training set = {len(train_dataset)}")
print(100*'-')
val_dataset = dataset_mod(drop_band_flag = False, drop_band_count = 0, rotate_flag = False, label_dir=labels_val, s1_dir=s1_train, s2_dir=s2_val, landcover_dir=LC_val, azimuth_dir=azimuth_val, wateroccurence_dir=wateroccurence_val, DEM_dir=ele_val)
print(f"Number of data samples in validation set = {len(val_dataset)}")
print(100*'-')
test_dataset = dataset_mod(drop_band_flag = True, drop_band_count = DROP_BAND_COUNT, rotate_flag = False, label_dir=labels_test, s1_dir=s1_train, s2_dir=s2_test, landcover_dir=LC_test, azimuth_dir=azimuth_test, wateroccurence_dir=wateroccurence_test, DEM_dir=ele_test)
...
with mlflow.start_run(run_id=run_id):
    ...
    for batch_idx in range(len(predictions_test)):
        ...
        for i in range(predictions_test[batch_idx].shape[0]):
            ...
            if current_index % 100 == 0:
                overlay_img.save(join(f"{local_save_dir}", "band_removal12", f"AMG_AD_BandDrop12_batch_{batch_idx}_sample_{i}_overlay.png"))
   ...
   # assert(False)
    ...
    metrics_df.to_csv(join(f"{local_save_dir}", "band_removal12", "AMGadaptiveDropout_Sen2_banDrop12.csv"))
