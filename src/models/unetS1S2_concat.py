...
class CloudSegmentationS1S2_concat(nn.Module):
    def __init__(
            self,  S2_input_channels: int = 13, S1_input_channels=3,
        ) -> None:

        super(CloudSegmentationS1S2_concat, self).__init__()
        ...
        self.dec_conv5 = conv_block(ch_in=768,ch_out=128)

        self.dec_upsample4 = up_conv(ch_in=128,ch_out=128)
        self.dec_conv4 = conv_block(ch_in=384,ch_out=64)

        self.dec_upsample3 = up_conv(ch_in=64,ch_out=64)
        self.dec_conv3 = conv_block(ch_in=192,ch_out=32)

        self.dec_upsample2 = up_conv(ch_in=32,ch_out=32)
        self.dec_conv2 = conv_block(ch_in=96,ch_out=16)
...
def main():
    ...
    arch = CloudSegmentationS1S2_concat()
    input_size = (
