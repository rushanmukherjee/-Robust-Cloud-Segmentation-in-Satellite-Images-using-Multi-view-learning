...
with mlflow.start_run(run_id=run_id):
    ...
    for batch_idx in range(len(predictions_test)):
        ...
        for i in range(predictions_test[batch_idx].shape[0]):
            ...
            if current_index % 100 == 0:
                overlay_img.save(join(f"{local_save_dir}", "2M_concat", f"test_batch_{batch_idx}_sample_{i}_overlay.png"))
   ...
   # assert(False)
    ...
    metrics_df.to_csv(join(f"{local_save_dir}", "2M_concat", "2MC_AD_testMetrics.csv"))
